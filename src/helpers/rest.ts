...
export function registerRestHandlers(app: Express): void {
	const handler = async (req: Request, res: Response): Promise<void> => {
		const { command: commandName, password, params: nestedParams, ...rest } = req.body

	// Combine flat and nested params, preferring nested if defined
	const params = typeof nestedParams === 'object' && nestedParams !== null
		? { ...rest, ...nestedParams }
		: { ...rest }

	if (!commandName || typeof CommandHandlers[commandName] !== 'function') {
		res.status(400).json({ command: commandName, error: `Unknown command '${commandName}'` })
		return
	}

	try {
		const result = await CommandHandlers[commandName](params, password, {
			emit: (event: any, data: any) => console.log(`[REST] Emitted ${event}`, data),
			platform: process.platform,
		})
		res.json({ command: commandName, status: 'ok', result })
	} catch (err: any) {
		console.error(`[REST] Command '${commandName}' failed:`, err)
		res.status(500).json({ command: commandName, error: err.message || 'Internal server error' })
	}
	}
